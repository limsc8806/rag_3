...
- **랭킹**: text/table/image는 개별 top-k를 가진 뒤 가중치 합산으로 병합됩니다.
  `config/settings.yaml`의 `top_k_table`, `weight_*`로 조절합니다.
- **쿼리 리라이트**: 기본은 규칙 기반이며, `rewrite_mode: "llm"` 설정 시
  LLM 기반 리라이트를 사용합니다.
- **답변 평가**: 기본은 휴리스틱이며, `generation.grade_mode: "llm"` 설정 시
  LLM grader를 사용합니다.
